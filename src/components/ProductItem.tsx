...
const ProductItem: React.FC<Props> = (props) =>  {
  let {
    product: { title, images, price, discount, availableForSale, quantityAvailable = 0 },
    onPress,
    containerStyle,
    imageStyle,
  } = props;
  ...
  let renderImage = () => {
    return (availableForSale) ? ( //Note when product is not for sale and Available qut is less then 0  //&& quantityAvailable > 0
      <View style={styles.imageContainer}>
        <Image style={[styles.image, imageStyle]} source={{ uri: images[0] || 'https://ncs.cd.gov.mn/wp-content/themes/icetheme/assets/images/no-image.png'}} />
      </View>
    ) : (
      <View style={styles.imageContainer}>
        <ImageBackground
