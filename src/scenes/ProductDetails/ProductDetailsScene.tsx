...
import ErrorToast from '../../core-ui/ErrorToast';
...
export default function ProductDetailsScene() {
  ...
  const [isErrorTostVisible, setIsErrorTostVisible] = useState(false)
  let [isWishlistActive, setWishlistActive] = useState(false);
  let [quantity, setQuantity] = useState(1);
   //Note Handle input changes and ensure the quantity is never less than 1
  ...
  let showErrorToast = (duration: number) => {
    setIsErrorTostVisible(true);
    setTimeout(() => {
      setIsErrorTostVisible(false);
    }, duration);
  };
  let hideErrorToast = () =>{
    setIsErrorTostVisible(false)
    ...
    <KeyboardAvoidingView keyboardVerticalOffset={bottomButtonHeight}>
      <View style={[styles.flex, isLandscape && styles.flexRow]}>
        ...
        <View style={styles.flex}>
          ...
          >
            <BottomActionBar
              isButtonDisabled={!productDetails.availableForSale} // || productDetails?.quantityAvailable <= 0
      ...
      <ErrorToast 
       data={{
        message: t('You have add minimum one quantity'),
        isVisible: isErrorTostVisible,
        hideErrorToast,
      }}
      />
      <ImageModal
